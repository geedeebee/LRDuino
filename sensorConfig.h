/////////////////////
// Suzuki RGV250 Config
//                              Sensor#  0           1            2          3          4        5         6
//                                    TC/i2c?     TC/i2c?        i2c        i2c      TC/i2c?   Dig:Hallx2  i2c
//                                     CHT-1       CHT-2        Tire-F    Tire-R     Ambient    Gear     Lean Ang 
...
int sensevals[] =                     { 25,       25,           25,       25,           40,       1,        0       }; // bogus start values for debugging
const uint8_t senseunits[] =          { 0,        0,            0,        0,            0,        4,        3       }; // 0 for C 1 for psi 2 for "OK" 3 for "deg", 4 for "count (number)"
const int sensemaxvals[] =            { 250,      250,          150,      150,          120,      6,        60      }; // the maximum scale to use in our graph for each sensor
const int8_t senseminvals[] =         { -10,      -10,          -10,      -10,          -10,      0,        -60     }; // the minimum scale to use in our graph for each sensor
int sensepeakvals[] =                 { 0,        0,             0,       0,            0,        0,        0       }; // somewhere for us to store *max* peak values
int sensepeakminvals[] =              { 0,        0,             0,       0,            0,        0,        0       }; // somewhere for us to store *min* peak values
const int sensewarnhivals[] =         { 80,       80,           100,      100,          120,      10,       55      }; // High value that we want to be warned if exceeded
const int sensewarnlowvals[] =        { 45,       45,           25,       25,           -10,      -999,     -55     }; // Low value that we want to be warned if exceeded (-999 means we don't care, do u?)
